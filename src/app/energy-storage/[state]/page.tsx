...
export async function generateMetadata({ params }: { params: Promise<{ state: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({
    title: `Home Battery Storage in ${stateData.name} (2026) | Compare Systems`,
    description: `Compare the top home battery storage systems available in ${stateData.name}. Tesla Powerwall, Enphase, Franklin WH, and more. See prices, specs, and how batteries pair with solar.`,
    path: `/energy-storage/${stateSlug}/`,
    keywords: [`battery storage ${stateData.name}`, `home battery ${stateData.name}`, `Tesla Powerwall ${stateData.name}`, `solar battery ${stateData.name}`],
  });
}

export default async function EnergyStorageStatePage({ params }: { params: Promise<{ state: string }> }) {
  const { state: stateSlug } = await params;
  const stateData = getStateBySlug(stateSlug);
        ...
        <div className="mt-4 lg:grid lg:grid-cols-3 lg:gap-12">
          ...
          <div className="lg:col-span-2">
            <h1 className="text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
              Home Battery Storage in {stateData.name} (2026)
            </h1>

            <p className="mt-4 text-lg leading-relaxed text-gray-600">
              Home battery storage systems allow {stateData.name} homeowners to store excess solar energy for use during
              evenings, outages, and peak-rate hours. Pairing a battery with your solar panels maximizes
              self-consumption, reduces grid dependence, and can provide whole-home backup power when you need it most.
              {stateData.electricityRate >= 0.20
                ? ` With ${stateData.name}'s high electricity rates of ${(stateData.electricityRate * 100).toFixed(1)} cents/kWh, battery storage is especially valuable for time-of-use rate arbitrage.`
                : stateData.netMetering
                  ? ` Combined with ${stateData.name}'s net metering program, batteries help you optimize when you use vs. export your solar energy.`
                  : ` In ${stateData.name}, where net metering options are limited, batteries help you maximize the value of every kilowatt-hour your solar panels produce.`}
            </p>

            {/* Battery Comparison Grid */}
            <section className="mt-10">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">
                Top Battery Systems for {stateData.name} Homes
              </h2>
            ...
            <section className="mt-12">
              <h2 className="text-2xl font-bold text-gray-900">
                Why Battery Storage in {stateData.name}?
              </h2>
              <div className="mt-4 space-y-4 text-gray-600 leading-relaxed">
                <p>
                  {stateData.name} homeowners have compelling reasons to add battery storage to their solar systems:
                </p>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="rounded-xl border border-gray-200 bg-white p-5">
                    ...
                    <p className="text-sm">
                      Store solar energy during the day and use it at night, reducing your reliance on the grid and
                  ...
                  <div className="rounded-xl border border-gray-200 bg-white p-5">
                    ...
                    <p className="text-sm">
                      Keep your lights, refrigerator, and essential systems running during power outages.
                      {stateData.slug === 'texas'
                        ? ' Especially important in Texas, where severe weather events can cause extended outages.'
                        : stateData.slug === 'hawaii'
                          ? ' Critical in Hawaii, where island grids can be vulnerable to storms and natural events.'
                          : stateData.slug === 'california'
                            ? ' Valuable in California, where Public Safety Power Shutoffs (PSPS) can cause planned outages.'
                            : ' Peace of mind knowing your home has a reliable backup power source.'}
                    </p>
                  </div>
                  ...
                  <div className="rounded-xl border border-gray-200 bg-white p-5">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <h3 className="font-semibold text-gray-900">Maximize Savings</h3>
                    </div>
                    <p className="text-sm">
                      {stateData.electricityRate >= 0.20
                        ? `With ${stateData.name}'s high rates of ${(stateData.electricityRate * 100).toFixed(1)} cents/kWh, using stored solar energy during peak hours can save you significantly more than exporting to the grid.`
                        : stateData.netMetering
                          ? `While ${stateData.name}'s net metering helps, a battery lets you strategically use or export energy to maximize your financial return.`
                          : `Without statewide net metering in ${stateData.name}, batteries help you use more of your own solar energy instead of buying from the grid.`}
                    </p>
                  </div>
            ...
            <section className="mt-12">
              <h2 className="text-2xl font-bold text-gray-900">
                Battery Storage Costs in {stateData.name}
              </h2>
